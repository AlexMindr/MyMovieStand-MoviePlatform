...
const ReviewAdd = ({ movieid, title }) => {
  ...
  useEffect(() => {
    if (reviews) {
      ...
      if (currentExists.length > 0) setIsCreateOrUpdate("update");
      else setIsCreateOrUpdate("create");
    }
  }, [movieid, reviews]);
  ...
  return (
    <>
      <Typography component="h2" variant="h3" className="Container-title">
        {isCreateOrUpdate.charAt(0).toUpperCase() + isCreateOrUpdate.slice(1)}{" "}
        review
      </Typography>
      <Box
        sx={{ flexGrow: 1, p: 2 }}
