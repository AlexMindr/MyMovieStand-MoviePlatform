...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    if (genresChecked && !allValEq(genresChecked, false)) {
      ...
      for (const [key, value] of Object.entries(genresChecked)) {
        if (value === true) checkedGen += key + ",";
      }
      checkedGen = checkedGen.slice(0, -1);
      if (checkedGen) toQuery.checked = checkedGen;
    }
    if (sorter && sorter !== "") toQuery.sorter = sorter;
    if (order && order !== "") toQuery.order = order;
    if (inputSearch && inputSearch !== "") toQuery.search = inputSearch;
    let query = "";
  ...
  return (
    <Paper elevation={2}>
      ...
      <Box className="container-movies">
                  ...
                  {movies.map((movie) => (
                    <li key={movie.movieid}>
                      <MovieList
                        ...
                        uscertification={movie.uscertification}
