    ...
    <Box className='Box-moviehomelist'>
        ...
        <Box className='moviehomelist-item'>
            ...
            <Box className='moviehomelist-title'>
                <Link to={`/movies/${movie.movieid}`}>{movie.title.length>25?movie.title.slice(0,25)+'...':movie.title}</Link>
                <span><PeopleAltIcon fontSize='small' sx={{verticalAlign:'text-bottom'}}/> {numFormatter(movie.popularity)}</span>
                <span><StarBorderIcon fontSize='small' sx={{verticalAlign:'text-bottom',color:'gold'}}/> {movie.rating?movie.rating:'N/A'}</span>
            </Box>
        </Box>
        <Box className='moviehomelist-status'> 
             <Button  onClick={handleOpen} variant='outlined' sx={{minWidth:'55px',padding:'2px 5px',color:bgColor?'white':'black',fontWeight:'bold',textTransform:'none',
            border:borderColor?borderColor:'1px solid black', backgroundColor:bgColor?bgColor:'white',textShadow:'1px 1px 1px black'}}>
                {status}
            </Button>
            <Modal
                              open={open}
                              ...
                              <Box className='watchformmodal'>
                                 <Box  sx={{width:'70vw',height:'90vh'}} component="div">
                                    <WatchlistForm movieid={parseInt(movie.movieid)} type={'movie'}
                                     handleCloseWatchForm={handleClose} title={movie.title} episodesTotal={1}/>
                                 </Box>
                              </Box>
                  </Modal>
        </Box>
