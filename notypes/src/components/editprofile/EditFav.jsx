...
const EditFav = (/*{watchlist,favourites,setFavourites,setWatchlist}*/) => {
  ...
  const onFavClick = (movieid) => {
    ...
    setError(null)
    const favtoAdd=watchlist.map(item=>item.movieid===movieid?({...item,favourite:true}):item).filter(item=>item.movieid===movieid)[0]
  ...
  const onRemClick = (movieid) => {
    const favtoRem=favourites.map(item=>item.movieid===movieid?({...item,favourite:false}):item).filter(item=>item.movieid===movieid)[0]
  ...
  return (
    <Box sx={{flexGrow:1}} className='watchlistfav-edit-div'>
      ...
      {error?
      <Typography component="h5" variant='h6' sx={{color:'red',marginBottom:1}}>{error.message}</Typography>
      :<></>
      }
